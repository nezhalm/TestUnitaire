...
public class DemandeService {
    ...
    public Optional<Demande> ajouterDemande(Demande d) {
        ...
        if (
            ...
            clientImp.chercher(d.getClient().getCode()).isPresent() && d.getPrice() > 0 && d.getDuration() > 0) {
            double mensualite = calculeMensualite(d.getDuration(),d.getPrice());
            d.setMonsualite(mensualite);
            result = demandeImp.ajouter(d);
        }
        return result;
    }
public Double calculeMensualite(Integer duration,Integer price){
    double tauxInteretMensuel = 0.12 / 12.0;
    double denominateur = 1 - Math.pow(1 + tauxInteretMensuel, -duration);
    double mensualite = price * tauxInteretMensuel / denominateur;
    // Format the result with two decimal places
    DecimalFormat df = new DecimalFormat("#.00");
    String formattedResult = df.format(mensualite);
    ...
    public boolean insertStatusHistory(String number) {
        boolean InsertDemandeUpdated = demandeImp.insertUpdateHistory(number);
        return InsertDemandeUpdated  ;
    }
}
